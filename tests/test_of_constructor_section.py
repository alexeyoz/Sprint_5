...
class TestConstructor:
    ...
    def test_sauces_section(self, driver):
        ...
        assert "current" in driver.find_element(*Locators.SAUCES_BUTTON).get_attribute("class")  # проверка перехода
    ...
    def test_toppings_section(self, driver):
        ...
        assert "current" in driver.find_element(*Locators.TOPPINGS_BUTTON).get_attribute("class")  # проверка перехода

    # 15. Проверка перехода к разделу "Булки"
    def test_bread_section(self, driver):
        driver.find_element(*Locators.SAUCES_BUTTON).click()
        driver.find_element(*Locators.BUNS_BUTTON).click()
        assert "current" in driver.find_element(*Locators.BUNS_BUTTON).get_attribute("class")  # проверка перехода
